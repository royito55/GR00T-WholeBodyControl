...
class LMPnPAppleToPlateStatic(LMSimpleEnv, DexMGConfigHelper):
    """
    Static version of apple to plate task.
...
class LMPnPAppleToPlateStaticDC(LMPnPAppleToPlateStatic):
    """
    Static version with LabArena (indoor lab environment).
    """
    pass  # The LabEnvMixin will be added when registering


class LMPnPCubeToZoneStatic(LMSimpleEnv, DexMGConfigHelper):
    """
    Static version with a red cube and blue target zone.
    Single table with cube and zone close together - no navigation required.
    Robot's lower body remains static throughout the task.
    """
    ...
    def _get_objects(self) -> list[SceneObject]:
        ...
        # Red cube - 6cm sided cube
        self.cube = SceneObject(
            ObjectConfig(
                name="red_cube",
                mjcf_path="objects/blocks/red_block.xml",  # Will create this
                static=False,
                scale=1.0,
                sampler_config=SamplingConfig(
                    x_range=np.array([0.0, 0.0]),  # Left side of table
                    y_range=np.array([0.05, 0.05]),
                    rotation=np.array([0.0, 0.0]),  # No rotation
                    reference_pos=np.array([0.4, 0, self.table.mj_obj.top_offset[2]]),
                ),
            )
        )
        
        # Blue zone - 40x40cm visual target area (no collision)
        self.zone = SceneObject(
            ObjectConfig(
                name="blue_zone",
                mjcf_path="objects/zones/blue_zone.xml",  # Will create this
                scale=1.0,
                static=True,  # Zone is fixed on the table
                sampler_config=SamplingConfig(
                    x_range=np.array([0.0, 0.0]),
                    y_range=np.array([-0.3, -0.3]),
                    rotation=np.array([0.0, 0.0]),
                    reference_pos=np.array([0.4, 0, self.table.mj_obj.top_offset[2] + 0.001]),  # Slightly above table
                ),
            )
        )
    ...
    def _get_success_criteria(self) -> SuccessCriteria:
        # Success when cube center is within 5cm of zone center (in xy plane)
        # Using IsClose instead of IsInContact since zone has no collision mesh
        return IsClose(self.cube, self.zone, max_distance=0.05, use_xy_only=True)
    ...
    def get_subtask_term_signals(self):
        # Signal when cube is lifted off the table
        obj_z = self.sim.data.body_xpos[self.obj_body_id(self.cube.mj_obj.name)][2]
        table_pos = self.sim.data.body_xpos[self.obj_body_id(self.table.mj_obj.name)]
        table_z = table_pos[2] + self.table.mj_obj.top_offset[2]
        return dict(cube_lifted=int(obj_z - table_z > self.LIFT_OFFSET))
...
class LMPnPCubeToZoneStaticDC(LMPnPCubeToZoneStatic):
    """
    Cube to zone static version with LabArena (indoor lab environment).
    """
    pass  # The LabEnvMixin will be added when registering
